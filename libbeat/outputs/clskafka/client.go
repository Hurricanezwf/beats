...
type client struct {
	...
	// BY HURRICANEZWF: 用于实现 CLS 日志上下文查询;
	nodeIP          string
	producerID      string
...
func newKafkaClient(
	...
	writer codec.Codec,
	cfg *sarama.Config,
	hostPrivateIPEnv string,
) (*client, error) {
	// 计算 producer ID, 为了后续支持在腾讯云上进行日志上下文查询.
	nodeIP := os.Getenv(hostPrivateIPEnv)
	if nodeIP == "" {
		return nil, fmt.Errorf("empty host_ip read from env %s", hostPrivateIPEnv)
	}
...
func (c *client) Publish(_ context.Context, batch publisher.Batch) error {
	...
	// BY HURRICANEZWF: 生成 package ID;
	packageID := c.generatePackageID()

	ch := c.producer.Input()
	for i := range events {
		d := &events[i]
		msg, err := c.getEventMessage(d, packageID)
		if err != nil {
		...
		msg.ref = ref
		msg.produceAt = time.Now() // 用于做监控统计;
...
func (c *client) getEventMessage(data *publisher.Event, packageID string) (*message, error) {
	event := &data.Content
	// BY HURRICANEZWF: 注入 package ID;
	event.Meta.Put("cls_package_id", packageID)
